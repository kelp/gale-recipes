...
1. This repo is the fetch index. Do not add `[build]`
   recipes, ledgers, or promote / verify-build CI.
2. Do not invent `tree_digest`. Admission records it.
3. `gale install` and `gale build` cannot work in an
   agent sandbox. They fail slowly. `just lint` and
   `just test` are the local gates. The real gate is
   CI's `test.yml` (`test` + `index-lint`) and
